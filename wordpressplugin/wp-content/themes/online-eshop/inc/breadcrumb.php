...
function online_eshop_breadcrumbs( $args = array() ) {

    $breadcrumb = apply_filters( 'online_eshop_breadcrumbs_object', null, $args );

    if ( !is_object( $breadcrumb ) )
        $breadcrumb = new Online_Eshop_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class Online_Eshop_Breadcrumb_Trail {
    ...
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            foreach ( $this->items as $item ) {
                ...
                // Trim the words if too larg
                $get_post_type = get_post_type( get_the_ID() );
                
                if( $get_post_type == 'product' && is_single() )
                    $item = __('Product Detail', 'online-eshop');
                else
                    if( is_single() )
                        $item = __('Blog Detail', 'online-eshop');
                    else
                        $item = $item;

                // Wrap the item text with appropriate itemprop.
                $item = !empty( $matches ) ? sprintf( '%s %s %s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<span itemprop="name">%s</span>', $item );
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => __( 'Browse:', 'online-eshop' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'online-eshop' ),
            'home'                => __( 'Home', 'online-eshop' ),
            'error_404'           => __( '404 Not Found', 'online-eshop' ),
            'archives'            => __( 'Archives', 'online-eshop' ),
            // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
            'search'              => __( 'Search results for &#8220;%s&#8221;',   'online-eshop' ),
            // Translators: %s is the page number.
            'paged'               => __( 'Page %s', 'online-eshop' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => __( 'Minute %s', 'online-eshop' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => __( 'Week %s', 'online-eshop' ),
    ...
    /**
     * Gets front items based on $online_eshop_wp_rewrite->front.
     *
     * @since  1.0.0
     * @access protected
     * @return void
     ...
     * @since  1.0.0
     * @access protected
     * @global object $online_eshop_wp_rewrite
     * @return void
     */
    protected function add_term_archive_items() {
        ...
        if ( false !== $taxonomy->rewrite ) {
            ...
            // If 'with_front' is true, dd $online_eshop_wp_rewrite->front to the trail.
    ...
    protected function add_post_type_archive_items() {
        ...
        if ( false !== $post_type_object->rewrite ) {
            ...
            // If 'with_front' is true, add $online_eshop_wp_rewrite->front to the trail.
    ...
    protected function add_user_archive_items() {
        global $online_eshop_wp_rewrite;

        // Add $online_eshop_wp_rewrite->front to the trail.
        $this->add_rewrite_front_items();
        ...
        if ( !empty( $online_eshop_wp_rewrite->author_base ) )
            $this->add_path_parents( $online_eshop_wp_rewrite->author_base );
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'online-eshop' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'online-eshop' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'online-eshop' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'online-eshop' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'online-eshop' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'online-eshop' ) ) );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'online-eshop' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'online-eshop' ) ) );
    ...
    protected function add_default_archive_items() {
        ...
        // If this is a date-/time-based archive, add $online_eshop_wp_rewrite->front to the trail.
        if ( is_date() || is_time() )
            $this->add_rewrite_front_items();
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'online-eshop' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'online-eshop' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'online-eshop' ) ) ) );
...
if( ! function_exists('online_eshop_header_title_display') ) {

    function online_eshop_header_title_display() {
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="breadcumb-cont">
            <?php $get_post_type = get_post_type( get_the_ID() ); ?>
            <?php wp_reset_postdata(); ?>
            <?php if(is_archive()) : ?>    
            <?php the_archive_title( '<h2>', '</h2>' ); ?>
            <?php elseif( is_home()) : ?>
            <h2><?php echo esc_html__('Blog','online-eshop'); ?></h2>
            <?php elseif(is_front_page()) : ?>
            <?php the_title( '<h2>', '</h2>' ); ?>
            <?php elseif(is_404()) : ?>
            <h2><?php echo esc_html__('404 Not Found','online-eshop'); ?></h2>
            <?php elseif(is_category()) : ?>
            <?php the_archive_title( '<h2>', '</h2>' ); ?>
            <?php elseif(is_search()) : 
            /* translators: %s: search term */?>
            <h2><?php printf( __( 'Search Results for: %s', 'online-eshop' ), get_search_query() ); ?></h2>
            <?php elseif(is_page()) : ?>
            <?php the_title( '<h2>', '</h2>' ); ?>
            <?php elseif( $get_post_type == 'product' && is_single()) : ?>
            <h2><?php echo esc_html__('Product Detail', 'online-eshop'); ?></h2>
            <?php elseif( $get_post_type == 'post' && is_single()) : ?>
            <h2><?php echo esc_html__('Blog Detail','online-eshop'); ?></h2>
            <?php endif;
            wp_reset_postdata();
            $breadcrumb_subtitle = $online_eshop_settings['online_eshop_breadcrumb_subtitle_display'];
            if( !empty( $breadcrumb_subtitle ) ) :
                printf( '<p>%1$s</p>', wp_kses_post($breadcrumb_subtitle) );
            else :
                printf( '<p>%1$s</p>', __( 'This is Photoshop`s version of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat.', 'online-eshop' ) );
            endif;
            ?>
        </div>
    </div>
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="breadcum-nav">
            ...
            <?php elseif( is_home()) : ?>
            <h4><?php esc_html_e('Blog','online-eshop'); ?></h4>
            <?php elseif(is_front_page()) : ?>
            <?php the_title( '<h4>', '</h4>' ); ?>
            <?php elseif(is_404()) : ?>
            <h4> <?php echo esc_html__('404 Not Found','online-eshop'); ?> </h4>
            <?php elseif(is_category()) : ?>
            <?php the_archive_title( '<h4>', '</h4>' ); ?>
            <?php elseif(is_search()) : 
            /* translators: %s: search term */?>
            <h4><?php printf( __( 'Search Results for: %s', 'online-eshop' ), get_search_query() ); ?></h4>
            <?php elseif(is_page()) : ?>
            <?php the_title( '<h4>', '</h4>' ); ?>
            <?php elseif( $get_post_type == 'product' && is_single()) : ?>
            <h4><?php echo esc_html__('Product Detail', 'online-eshop'); ?></h4>
            <?php elseif( $get_post_type == 'post' && is_single()) : ?>
            <h4><?php echo esc_html__('Blog Detail','online-eshop'); ?></h4>
            <?php endif;
            wp_reset_postdata();

            online_eshop_breadcrumbs(); ?>
        </div>
    </div>
    <?php
    }
